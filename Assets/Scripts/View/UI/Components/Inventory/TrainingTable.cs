...
namespace View.Components.Inventory
{
    ...
    /// </summary>
    public class TrainingTableComponent : VisualElement
    {
        private VisualElement _tableBody;
        private Label _header;
        
        private Dictionary<UnitType, Label> _currentRows = new ();
        ...
        public void SetProduction(SelectionPanel selectionPanel, TrainingBuilding trainingBuilding)
        {
            ...
            foreach (var job in trainingBuilding.TrainingJobs)
            {
                var unitType = job.UnitType; 
                
                var row = CreateRow("data-row");
                
                // Column 1: Name
                var nameLabel = row.CreateChild<Label>("item-name");
                nameLabel.text = unitType.ToString();
                ...
                // Initial data set
                outgoingLabel.text = job.Quantity.ToString(); 
